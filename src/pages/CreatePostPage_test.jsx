...
function CreatePostPage() {
  ...
  // tat data api
  const getTatData = async (keyword) => {
    const VITE_TAT_DATA = import.meta.env.VITE_TAT_DATA;
    console.log(VITE_TAT_DATA)

    try {
      const url = `https://tatdataapi.io/api/v1/places?keyword=${keyword}&categoryCode=attraction`;
      const { data } = await axios.get(url, {
        headers: {
          Accept: "application/json",
          "Accept-Language": "en",
          "x-api-key": VITE_TAT_DATA,
        },
      });
      console.log(data);
      return data;
  ...
  useEffect(() => {
    const loadToxicityModel = async () => {
      const loadedModel = await toxicity.load(0.90);
